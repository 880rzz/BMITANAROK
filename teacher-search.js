...
function removeLegacyProfilePrograms(){document.querySelectorAll('article.profile .programs').forEach(function(el){el.remove()})}
function removePartnerProgramsVisualBlock(){Array.from(document.querySelectorAll('section')).forEach(function(sec){var txt=slugText(sec.textContent||'');if(txt.indexOf('partnerprogramjaink egy helyen')>=0||txt.indexOf('bmi partner programok')>=0)sec.remove()})}
function syncProfileProgramLinks(cfg,teachers){
 teachers.forEach(function(t){
  ...
  list.innerHTML='';
  programs.forEach(function(p){if(!p.url)return;var a=document.createElement('a');a.href=p.url;a.target='_blank';a.rel='noopener';a.textContent=p.name;list.appendChild(a)})
 })
}
function setProfileOpen(profile,open){profile.classList.toggle('bmi-profile-collapsed',!open);var head=profile.querySelector('.profile-head');if(head)head.setAttribute('aria-expanded',open?'true':'false')}
function setupProfileAccordions(){
 document.querySelectorAll('article.profile').forEach(function(profile){
  var head=profile.querySelector('.profile-head');if(!head||head.dataset.accordionReady)return;
  head.dataset.accordionReady='1';head.setAttribute('role','button');head.setAttribute('tabindex','0');head.setAttribute('aria-expanded','false');
  var marker=document.createElement('span');marker.className='bmi-accordion-marker';marker.setAttribute('aria-hidden','true');marker.textContent='+';head.appendChild(marker);
  function toggle(e){if(e&&e.target&&e.target.closest&&e.target.closest('a,button'))return;setProfileOpen(profile,profile.classList.contains('bmi-profile-collapsed'))}
  head.addEventListener('click',toggle);head.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle(e)}});
  setProfileOpen(profile,false);
 });
 function openHash(){var id=decodeURIComponent(location.hash||'').replace(/^#/,'');if(!id)return;var profile=document.getElementById(id);if(profile&&profile.matches('article.profile')){setProfileOpen(profile,true);setTimeout(function(){profile.scrollIntoView({block:'start'})},0)}}
 openHash();window.addEventListener('hashchange',openHash);
}
function setupFinderAccordion(sec){
 var head=sec&&sec.querySelector('.ts-head');if(!head||head.dataset.accordionReady)return;
 head.dataset.accordionReady='1';head.setAttribute('role','button');head.setAttribute('tabindex','0');head.setAttribute('aria-expanded','false');
 var marker=document.createElement('span');marker.className='bmi-accordion-marker';marker.setAttribute('aria-hidden','true');marker.textContent='+';head.appendChild(marker);
 sec.classList.add('bmi-finder-collapsed');
 function toggle(e){if(e&&e.target&&e.target.closest&&e.target.closest('a,button'))return;var open=sec.classList.contains('bmi-finder-collapsed');sec.classList.toggle('bmi-finder-collapsed',!open);head.setAttribute('aria-expanded',open?'true':'false')}
 head.addEventListener('click',toggle);head.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle(e)}});
}
function normalizeUi(){
 removeLegacyProfilePrograms();removePartnerProgramsVisualBlock();
...
function init(){
 normalizeUi();
 var cfg=window.BMI_FINDER;if(!cfg||!Array.isArray(cfg.programs))return;if(window.BMI_PROGRAM_TAXONOMY&&window.BMI_PROGRAM_TAXONOMY.apply)window.BMI_PROGRAM_TAXONOMY.apply();refreshCategories();var teachers=teacherCards();syncProfileProgramLinks(cfg,teachers);setupProfileAccordions();if(CATEGORIES.length!==6)return;
 ...
 results.innerHTML='';setupFinderAccordion(sec);
}
